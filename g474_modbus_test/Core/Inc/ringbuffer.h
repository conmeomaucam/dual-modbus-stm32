...
/**
 * [TIẾNG VIỆT] Cấu trúc Bộ đệm vòng (Ring Buffer / Circular Buffer)
 * Vai trò: Được sử dụng để làm bộ đệm trung gian nhận dữ liệu nối tiếp UART bất đồng bộ (ví dụ qua ngắt RX hoặc DMA).
 *          Nó giúp lưu trữ dữ liệu tạm thời trước khi được lấy ra xử lý bởi hàm agile_modbus_slave_handle,
 *          tránh mất mát dữ liệu khi Master gửi liên tục.
 * Cơ chế hoạt động: Sử dụng kỹ thuật gương (mirroring) qua bit MSB để phân biệt trạng thái đầy (Full) hay rỗng (Empty)
 *          mà không cần bớt đi 1 ô nhớ.
 */
struct rt_ringbuffer {
    uint8_t *buffer_ptr; /* Con trỏ trỏ tới vùng nhớ đệm */
